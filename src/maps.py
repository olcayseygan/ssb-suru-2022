...
class Tile:
    ...
    def draw_terrain(self, surface):
        self.terrain_surf = pygame.image.load(
            terrain_images[TERRAIN_MAP[self.terrain]]).convert()
...
class GameMap:
    ...
    def generateMap(self, map_config):
        ...
        self.map_terrain = numpy.zeros(
            (self.size_y, self.size_x), dtype=numpy.int32)
        ...
        for i in range(self.size_y):
            for j in range(self.size_x):
                self.position = (self.x+j*self.x_step, self.y -
                                 (j % 2)*self.y_step+2*i*self.y_step)
    ...
    def getDistance(self, pos_1, pos_2):
        ...
        distance = (abs(pos1[0]-pos2[0]) + abs(pos1[1]-pos2[1]
                                               ) + abs(pos1[0]+pos1[1]-pos2[0]-pos2[1]))//2
        return distance
