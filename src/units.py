...
class Unit(pygame.sprite.Sprite):
    def __init__(self, gmap, x=0, y=0, offset=(0, 0), id_num=0):
        ...
        if self.render:
            ...
            if not self.resource:
                self.wreck = pygame.image.load(
                    "data/images/wreck_"+str(self.team)+".png").convert()
    ...
    def moveAction(self, m_action=0, target=0):
        ...
        if not invalid:
            ...
            if self.gmap.game.render:
                self.setAnimationMoveTarget(x, y)
    ...
    def shoot(self, idd):
        ...
        if target.fly and not self.aa:
            ...
            return
        distance = self.gmap.getDistance([target.x_coor, target.y_coor], [
    ...
    def __init__(self, gmap, x=0, y=0, team=0, id_num=0):
        ...
        if gmap.game.render:
            self.base_surf = pygame.image.load(
                "data/images/tankh_"+str(self.team)+".png").convert_alpha()
        ...
        if gmap.game.render:
            self.animations['shoot'] = pygame.image.load(
                "data/images/fire_tank.png").convert_alpha()
...
class LightTank(Unit):
    def __init__(self, gmap, x=0, y=0, team=0, id_num=0):
        ...
        if gmap.game.render:
            ...
            self.base_surf = pygame.image.load(
                "data/images/tankl_"+str(self.team)+".png").convert_alpha()
            self.base_surf = pygame.transform.scale(self.base_surf, (50, 50))
...
class Truck(Unit):
    def __init__(self, gmap, x=0, y=0, team=0, id_num=0):
        ...
        if gmap.game.render:
            self.base_surf = pygame.image.load(
                "data/images/truck_"+str(self.team)+".png").convert_alpha()
            self.base_surf = pygame.transform.scale(self.base_surf, (50, 50))
            #self.surf.set_colorkey((255, 255, 255), RLEACCEL)
            self.surf = copy(self.base_surf)
            self.rect = self.surf.get_rect()
...
class Drone(Unit):
    def __init__(self, gmap, x=0, y=0, team=0, id_num=0):
        ...
        if gmap.game.render:
            self.base_surf = pygame.image.load(
                "data/images/drone_"+str(self.team)+".png").convert_alpha()
            self.base_surf = pygame.transform.scale(self.base_surf, (50, 50))
            #self.surf.set_colorkey((255, 255, 255), RLEACCEL)
            self.surf = copy(self.base_surf)
            self.rect = self.surf.get_rect()
...
class Base(Unit):
    def __init__(self, gmap, x=0, y=0, team=0, id_num=0):
        ...
        if gmap.game.render:
            self.surf = pygame.image.load(
                "data/images/base_"+str(self.team)+".png").convert_alpha()
    ...
    def trainUnit(self, train_unit):
        ...
        if not self.getTile().hasUnit() and self.unloaded >= cost:
            y, x = self.getCoor()
            self.gmap.game.units[self.team].append(UNIT_MAP_KEY_TO_CLASS_MAP[train_unit](
                self.gmap, x, y, team=self.team, id_num=self.gmap.game.idGeneretor()))
            self.unloaded -= cost
        else:
