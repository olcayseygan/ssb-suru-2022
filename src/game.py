...
from pygame.locals import (K_BACKSPACE, K_ESCAPE, K_KP_ENTER, K_QUOTEDBL,
                           K_RETURN, K_SPACE, K_TAB, KEYDOWN, QUIT)
...
class Game:
    ...
    def __init__(self, args, agents):
        ...
        self.agents = []
        signal.signal(signal.SIGINT, self._agent_timeout_handler)
        self.config = None
        ...
        if self.render:
            pygame.time.set_timer(self.GAMESTEP, self.turn_timer)
        self.ANIMATIONSTEP = pygame.USEREVENT + 2
        if self.render:
            pygame.time.set_timer(self.ANIMATIONSTEP, 10)
        ...
        if self.render:
            self.screen = pygame.display.set_mode(
                (self.SCREEN_WIDTH + self.human_console_size, self.SCREEN_HEIGHT))
        if self.render:
    ...
    def step(self, action):
        ...
        reward = self.bases[self.go_team].getScore(
        ) - self.bases[(self.go_team+1) % 2].getScore()
    ...
    def reset(self, agents=None, switch_sides=False, reload_agents=False):
        ...
        base = blue['base']
        self.bases.append(
            Base(self.gmap, base['x'], base['y'], team=0, id_num=self.idGeneretor()))
        for u in blue['units']:
            self.units[0].append(UNIT_TYPE_TO_CLASS_MAP[u['type']](
                self.gmap, u['x'], u['y'], team=0, id_num=self.idGeneretor()))

        # Red
        red = self.config['red']
        base = red['base']
        self.bases.append(
            Base(self.gmap, base['x'], base['y'], team=1, id_num=self.idGeneretor()))
        for u in red['units']:
            self.units[1].append(UNIT_TYPE_TO_CLASS_MAP[u['type']](
                self.gmap, u['x'], u['y'], team=1, id_num=self.idGeneretor()))

        # Resources
        for resource in self.config['resources']:
            self.resources.append(
                Resource(self.gmap, resource['x'], resource['y'], id_num=self.idGeneretor()))
        self.all_ubr = [self.units, self.bases, self.resources]
        self.num_of_units = len(self.units[0])
        self.state_space = [numpy.zeros(
            [2]), numpy.zeros([self.map_x, self.map_y])]
        ...
        if agents or reload_agents or not len(self.agents):
            ...
            for i in range(2):
                if self.agents_classes[i] == None:
                    ...
                    self.agents.append(
                        self.agents_classes[i](i, self.num_of_units))
    ...
    def sim(self):
        ...
        while self.running and self.turn <= self.max_turn:
            ...
            if (self.gif or self.img) and not ss:
                ss = True
                pil_string_image = pygame.image.tostring(
                    self.screen, "RGB", False)
                pil_image = Image.frombytes(
                    'RGB', self.screen.get_size(), pil_string_image, 'raw')
                ss_list.append(pil_image)
                if self.img:
                    pygame.image.save(
                        self.screen, "screenshot"+str(self.turn)+"_"+str(self.go_team)+".jpg")
            #printProgressBar(self.turn, self.max_turn, prefix = 'Progress:', suffix = 'Complete', length = 50)
            print('\rProgress: {}%'.format(
                int((self.turn/self.max_turn)*100)), end='\r')
        if self.gif:
            ss_list[0].save(fp="game.gif", format='GIF',
                            append_images=ss_list, save_all=True, duration=200, loop=0)
    ...
    def renderGame(self):
        if self.render:
            ...
            if self.has_human:
                self.screen.blit(self.gmap.console_surface,
                                 (self.SCREEN_WIDTH, 0))
            u1, u2 = self.units
            ...
            self.screen.blit(
                turn_surf, (self.SCREEN_WIDTH-100, self.SCREEN_HEIGHT-20))
            if self.game_over:
                game_over_string = 'Game Over'
                game_over_surf = self.font.render(
                    game_over_string, True, (0, 0, 0))
                self.screen.blit(
                    game_over_surf, (self.SCREEN_WIDTH-100, self.SCREEN_HEIGHT-40))
